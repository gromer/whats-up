...
function BuildDetails() {
  ...
  const build = component?.buildPipelines.find(build => build.id === buildId);

  if (!component || !build) {
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-gray-950 flex items-center justify-center">
        <div className="text-center">
          <h2 className="text-2xl font-semibold text-gray-900 dark:text-white">Build not found</h2>
          <button
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="bg-white dark:bg-gray-900 rounded-lg shadow-sm border border-gray-200 dark:border-gray-800 p-6 mb-8">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{build.name}</h1>
              <button
            ...
            <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${getStatusClasses(build.status)}`}>
              {build.status}
            </span>
          </div>
          ...
          <div className="border-t border-gray-200 dark:border-gray-800 pt-6">
            <dl className="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2">
              ...
              <div>
                <dt className="text-sm font-medium text-gray-500 dark:text-gray-400">Build ID</dt>
                <dd className="mt-1 text-sm text-gray-900 dark:text-white">{build.id}</dd>
              </div>
            </dl>
          </div>
        </div>
